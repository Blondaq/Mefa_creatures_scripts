class mefa_Gondor_Anorien_archers_upgraded : mefa_Humanoid_Logic_Base {

	protected float         m_Scale = 1.1;
	void mefa_Gondor_Anorien_archers_upgraded()
	{
	...
	*/
	...
	void FireProjectile(vector targetPosition) {
		vector rightHandPosition = this.ModelToWorld(this.GetMemoryPointPos("firingFromBowZed"));
		Object projectile;
		projectile = Object.Cast(GetGame().CreateObject("mefa_Anorien_Archers_Arrow_Fire_Upgrade", rightHandPosition));
		...
		if (projectile) {
			...
			float hitChance = 0.7; // Nastavte to na hodnotu, kterou chcete pro �anci na z�sah (0.5 pro 50%)
	...
	override bool FightAttackLogic(int pCurrentCommandID, DayZInfectedInputController pInputController, float pDt)
	{
		...
		m_ActualAttackType = GetDayZInfectedType().ChooseAttack(DayZInfectedAttackGroupType.FIGHT, targetDist, pitch);
		{
			...
			if (targetDist < 35.0 && targetDist > 1.8)
			{
			...
			{
				StartCommand_Attack(m_ActualTarget, 2, 0);
